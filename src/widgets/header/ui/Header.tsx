...
import { SplitModeSelect, useTabStore } from "@/widgets/contents-tab-bar";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function Header() {
  ...
  const { user, availableMenu, headerMenus, logout } = useAuthStore();
  const { splitMode, setSplitMode, activeHeaderCategories, addTab, updateHeaderCategories } = useTabStore();

  const activeTopNo = useMemo(
    () => (pathname ? findTopByPath(pathname, availableMenu)?.menuNo ?? null : null),
    [pathname, availableMenu]
  );

  const onHeaderClick = (menuNo: string) => {
    // 사이드바 필터링 설정
      ...
      <nav className="flex items-center justify-between px-6 py-3">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className="text-xl font-bold text-gray-900">상조업무관리</span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-1">
          {headerMenus.map((m) => {
            ...
            return (
              <button
                key={m.menuNo}
                onClick={() => onHeaderClick(m.menuNo)}
                ...
                // }`}
                className={`group relative flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl text-xs font-medium transition-all duration-200 min-w-[80px] ${isActive
              ...
              >
                ...
                <div className={`relative flex items-center justify-center w-10 h-10 rounded-lg transition-colors ${isActive
                  ? "bg-blue-100"
                  : "bg-gray-100 group-hover:bg-gray-200"
                }`}>
                  {m.iconUrl ? (
                ...
                {/* 호버 효과 */}
                <div className={`absolute inset-0 rounded-xl transition-opacity ${isActive ? "opacity-0" : "opacity-0 group-hover:opacity-5 group-hover:bg-gray-900"}`} />
              </button>
            );
          })}
        </div>
