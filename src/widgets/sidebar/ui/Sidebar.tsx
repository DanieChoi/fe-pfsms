...
export default function Sidebar() {
  ...
  const { availableMenu } = useAuthStore();
  ...
  // 경로 변경 → 활성/확장 동기화
  useEffect(() => {
    if (pathname) setFromPath(pathname, availableMenu);
  }, [pathname, availableMenu, setFromPath]);
  ...
  // 메뉴 클릭 핸들러
  const handleMenuClick = useCallback((menu: IMenuItem, hasSubMenu: boolean) => {
    if (hasSubMenu) {
      // 하위 메뉴가 있으면 토글만
      toggle(menu.menuNo);
      ...
      // 탭 추가 (Zustand 스토어 사용)
      if (menu) {
        const View = resolveViewByHref(menu.menuHref || undefined) || undefined;
        addTab({
          id: menu.menuNo,
          label: menu.menuNm,
          href: menu.menuHref,
      ...
      // href가 있으면 페이지 이동
      if (menu.menuHref && menu.menuHref.trim() !== "") {
        router.push('/contents/' + menu.menuHref);
      }
    }
  }, [toggle, addTab, router]);

  // 현재 활성 메뉴인지 정확히 판단
  const isActiveMenu = useCallback((menu: IMenuItem): boolean => {
    return activeLeafNo === menu.menuNo || activeTopNo === menu.menuNo;
  }, [activeLeafNo, activeTopNo]);

  // 하위에 활성 메뉴가 있는지 판단
  const hasActiveDescendant = useCallback((node: IMenuItem): boolean => {
    if (isActiveMenu(node)) return true;
    return node.subMenu?.some((child) => hasActiveDescendant(child)) ?? false;
  }, [isActiveMenu]);

  const renderMenu = (menu: IMenuItem, level = 0) => {
    const isTop = level === 0;
          ...
          {!isCollapsed && (
            <>
              <div className="p-4 border-b border-slate-600 flex-shrink-0">
                <h2 className="text-lg font-semibold text-gray-100">{availableMenu.find(m => m.menuNo === filteredTopNo)?.menuNm}</h2>
              </div>
              <div className="flex-1 p-3 overflow-y-auto min-h-0">
                ...
                {/*filteredTopNo && (
                  <div className="mb-3 p-2 bg-slate-700/50 rounded text-sm">
                    <div className="flex items-center justify-between text-gray-300">
                      <span>필터링 중: {availableMenu.find(m => m.menuNo === filteredTopNo)?.menuNm}</span>
                      <button
                ...
                {availableMenu
                  ...
                  .map((menu) => renderMenu(menu as IMenuItem, 0))
                }
              </div>
            </>
          )}
